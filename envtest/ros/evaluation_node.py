...
class Evaluator:
    def __init__(self, config, exp_name):
        ...
        with open('/home/anish/evfly_ws/src/evfly/flightmare/flightpy/configs/vision/config.yaml') as file:
            config_params = yaml.load(file, Loader=yaml.FullLoader)
        self.is_trees = 'trees' in config_params['environment']['level']
        self.quad_radius = config_params['quad_radius']
        print(f'[EVALUATOR_NODE] is_trees = {self.is_trees}')
    ...
    def callbackObstacles(self, msg):
        ...
        if margin < 0:
            if not self.hit_obstacle:
                self.crash += 1
                print()
                print(f"[EVALUATOR_NODE] Crashed (quadrotor radius {self.quad_radius})!")
                print()
